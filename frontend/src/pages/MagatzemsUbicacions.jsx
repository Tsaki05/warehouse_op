...
export default function MagatzemsUbicacions() {
  ...
  useEffect(() => {
    ...
    const params = magIds.length > 0 ? { magatzem_filter: magIds } : undefined;
    getMagatzems(params)
      .then(res => {
  ...
  if (loading) return <div className="state-box">Carregant magatzems...</div>;
  if (!magatzems.length) return <div className="state-box">No tens cap magatzem assignat.</div>;
